...
# 1. 获取当前脚本所在的绝对路径 (exp/simulation)
...
def parse_total_runtime(output_str):
    """
    从 exp_engine.py 的标准输出中解析 Total Runtime。
    """
...
def run_single_benchmark(benchmark_filename, run_id):
    """
    运行单个 benchmark 并返回 (status, value)
    """
...
def write_latex_file(results):
    ...
    try:
        with open(TEX_FILE, "w", encoding="utf-8") as f:
            ...
            for r in results:
                # 使用 LaTeX 的 \scriptsize 缩小 IQR 的字体，使其更美观
