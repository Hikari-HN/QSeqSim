...
def init_log(n):
    ...
    """
    log_file = os.path.join(LOG_DIR, f"qrw_{n}.log")
    # Open file in 'w' mode to clear existing content (create if not exists)
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}] Quantum Random Walk experiment log (n={n}) initialized\n")

def write_log(n, content):
    """
    Write content to specific log file (qrw_{n}.log) with timestamp
    :param n: Number of qubits (for log file naming)
...
def run_qrw_experiment(n, it, report_iters=None, timeout_seconds=TIMEOUT_SECONDS):
    ...
    try:
        ...
        for result_val, input_basis in zip(result_list, input_basis_list):
            ...
            # Core QRW calculation logic
            Sim.init_input_state_by_basis(input_basis)
            Sim.init_comb_bdd()
            
            # Quantum Random Walk operations
            Sim.H(1)
...
def main():
    ...
    for n, it in configs:
        ...
        write_log(n, "="*50)
        write_log(n, f"Quantum Random Walk experiment (n={n}, it={it}) finished")
        write_log(n, "="*50)
