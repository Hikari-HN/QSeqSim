...
# class GateOp:
#     """原子操作：门或测量"""
#     def __init__(self, name: str, qubits: List[int], params: List[float] | None = None, c_targets: List[int] | None = None):
#         self.name = name
...
#         return f"Op({self.name}, q={self.qubits}{params_str}{c_str})"
...
#         self.type = 'DQC'
#         self.target_clbits = target_clbits # List of indices (支持多比特寄存器)
#         self.cases = cases
...
#         return self._process_statements(program.statements)
...
#         # Case 1: IndexedIdentifier (左值 c[0])
...
#         return GateOp("measure", qubits, c_targets=c_targets)

#     def _parse_condition_expr(self, node: Any) -> Dict:
#         """
#         解析条件表达式。
#         返回 {'indices': [int, ...], 'value': int}
#         支持单比特 (c[0]) 和 整个寄存器 (c)。
#         """
#         info = {'indices': [], 'value': 1} 
        
#         def get_indices(n):
#             # 1. IndexedIdentifier (c[0])
...
#             # 3. IndexExpression (c[0] as rvalue)
...
#         elif isinstance(node, ast.UnaryExpression): # !c
...
#         # Fix: QASM range is inclusive [start, end], Python range is [start, end)
#         try:
#             start_val = int(start)
#             stop_val = int(stop)
#             step_val = int(step)
...
class GateOp:
    ...
    def __init__(
        ...
        is_final_measure: bool = False,  # 是否被标记为末态测量
    ):
        self.name = name
        self.qubits = qubits  # [Global Integer Indices]
        self.params = params if params is not None else []
...
class QiskitParser:
    ...
    # ==========================================
    # 3. IR 级别的“末态测量”全局标记 pass
    # ==========================================

    def _mark_final_measurements(self, blocks: list):
        """
        在 IR 层面对所有 GateOp("measure") 做一次全局标记：
        - 对每个 qubit，看它在整个程序顺序中的“最后一次操作”；
          若最后一次操作是测量，且该测量写入的经典比特不参与控制流条件，则 is_final_measure=True。
        - 否则为 mid-measure。
        """

        # 1. 收集所有“参与控制流条件”的经典比特
        ...
        # 2. 全局时间线：为每个 GateOp 分配一个全局 op_id，收集 (qubit, op_id, op_ref)
        #    注意：即使在分支/循环内部，我们也按 DFS 顺序简单线性化。
        ...
        # 3. 对每个 qubit 找“最后一次 op”，若是测量且不参与控制流，则标记为 final
        for q, usage_list in qubit_usages.items():
            ...
            # 如果该测量写入了控制流经典位，则强制 mid-measure
